                  ...
                  <div class="modal-body">
                    <div class="d-flex flex-row justify-content-between">
                      <div>
                        ...
                        <p>Mã đơn hàng: <span class="order-id">{{ $order->order_id }}</span></p>
                        <p>
                          Ngày tạo đơn hàng:
                          <span class="order-created-at">{{ $order->created_at }}</span>
                        </p>
                        <p>
                          Trạng thái hiện tại:
                      ...
                      <div>
                        ...
                        <p>
                          Tên người nhận:
                          <span class="order-">{{ $order->name }}</span>
                        </p>
                        <p>
                          Số điện thoại:
                          <span class="order-id">{{ $order->phone_number }}</span>
                        </p>
                        <p>
                          Địa chỉ nhận hàng:
                          <span class="order-id"
                    ...
                    <div>
                      ...
                      <div
                        class="media border-bottom py-2 px-3 d-flex justify-content-between"
                      >
                        <div class="media-left d-flex">
                          ...
                          <div class="mx-3">
                            <p class="media-heading">
                              <small>{{ $product->product->product_name }}</small>
                            </p>
                            <p><small>x {{ $product->quantity }}</small></p>
                          </div>
                        </div>
                        <div class="media-right">
                          <p>{{ number_format($product->product->price * $product->quantity, 0, '', ',') }} VNĐ</p>
                        </div>
                      </div> 
                      @endforeach      
                      <p>
                        Phí giao hàng:
                        <span style="font-weight: bold;"> {{ number_format($order->delivery_fee, 0, '', ',') }} VNĐ</span>
                      </p>
                      <p>
                        Ghi chú:
                        <?php
                        ...
                        ?>
                            <span class="order-note">Không có ghi chú đơn hàng</span>
                        <?php
                        }
                        ?>
                      </p>
                      <p>
                        ...
                        ?>
                            <span style="font-weight: bold;"> Thanh toán qua {{ $order->payment }}</span>
                        <?php
                        } else {
                        ?>
                            <span style="font-weight: bold;"> {{ $order->payment }} </span>
                        <?php
                        }
                        ?>
                      </p>
                      <p>
                        ...
                        <?php
                        if (isset($order->payment_at)) {
                        ?>
                            <span class="order-note"> {{ $order->payment_at }}</span>
                        <?php
                        } else {
                        ?>
                            <span class="order-note"> Chưa thanh toán</span>
                        <?php
                        }
                        ?>
                      </p>
                      <p class="text-end">
                        Tổng tiền:
                        <strong class="order-total-fee" style="font-size: 40px"
                          >{{ number_format($product->product->price * $product->quantity + $order->delivery_fee, 0, '', ',') }} VNĐ</strong
                        >
                      </p>
                    </div>
                  </div>
